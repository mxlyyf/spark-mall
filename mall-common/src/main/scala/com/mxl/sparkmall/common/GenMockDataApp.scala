package com.mxl.sparkmall.common

import java.text.SimpleDateFormat
import java.util.UUID

import com.mxl.sparkmall.common.bean.{CityInfo, ProductInfo, UserInfo, UserVisitAction}
import com.mxl.sparkmall.common.util.{Constans, RandomOptions, RandomUtil}
import org.apache.spark.SparkContext
import org.apache.spark.sql.{DataFrame, SparkSession}

import scala.collection.mutable.ListBuffer

object GenMockDataApp {
  // --- 用户相关参数 开始 ---
  // 用户数量
  val userNum = 100
  // 年龄范围
  ...
  def mockUserInfo: List[UserInfo] = {
    ...
    (1 to userNum).toList.map(
      i => new UserInfo(i,
        s"user_$i",
        s"name_$i",
        RandomUtil.randomInt(ageFrom, ageTo),
    ...
    for (i <- 1 to sessionNum) {
      ...
      while (!isQuit) {
        ...
        if (action == "quit") {
          ...
          val cityId: Long = RandomUtil.randomLong(1, 26)
          action match {
            case "search" => searchKeyword = searchKeywordsOpts.getRandomOption()
            case "click" => {
              clickCategoryId = RandomUtil.randomInt(1, cargoryNum)
              clickProductId = RandomUtil.randomInt(1, productNum)
            }
            case "order" => { // 1,2,3,7
              orderCategoryIds = RandomUtil.randomMultiInt(1, cargoryNum, RandomUtil.randomInt(1, 5), false).mkString(",")
              orderProductIds = RandomUtil.randomMultiInt(1, productNum, RandomUtil.randomInt(1, 3), false).mkString(",")
            }
            case "pay" => {
              payCategoryIds = RandomUtil.randomMultiInt(1, cargoryNum, RandomUtil.randomInt(1, 5), false).mkString(",")
              payProductIds = RandomUtil.randomMultiInt(1, productNum, RandomUtil.randomInt(1, 3), false).mkString(",")
            }
          }
          rows += UserVisitAction(actionDateString,
            userId,
            sessionId,
            RandomUtil.randomInt(1, pageNum),
            actionTimeString,
            searchKeyword,
            clickCategoryId,
            clickProductId,
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark: SparkSession = SparkSession
      .builder()
      .master("local[1]")
      .appName("GenMockDataApp")
      .enableHiveSupport()
      .config("spark.sql.warehouse.dir", "hdfs://hadoop101:9000/warehouse/sparkmall")
      .getOrCreate()
