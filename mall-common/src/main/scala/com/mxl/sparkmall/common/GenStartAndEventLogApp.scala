...
import com.mxl.sparkmall.common.util.{Okhttp3Client, RandomOptions, RandomUtil}
...
object GenStartAndEventLogApp {
  ...
  def initOneStartupLog(): String = {
     ...
     */
    val mid: String = "mid_" + RandomUtil.randomInt(1, 500)
    val uid: String = "" + RandomUtil.randomInt(1, 10000)
  ...
  def initOneEventLog(startupLogJson: String) = {
    ...
    eventLogObj.put("pageId", RandomUtil.randomInt(1, 50))
    eventLogObj.put("nextPageId", RandomUtil.randomInt(1, 50))
    eventLogObj.put("itemId", RandomUtil.randomInt(1, 50))
    eventLogObj.toJSONString
  }

  // 开始生成日志
  def generateLog(): Unit = {
    val url = "http://192.168.213.102/log"
    //val url = "http://127.0.0.1:8080/log"
    (0 to startupNum).foreach(_ => {
      // 生成一条启动日志
      val oneStartupLog: String = initOneStartupLog()
      // 发送启动日志
      Okhttp3Client.post(url, oneStartupLog)
      // 模拟出来多条事件日志
      while (!quitOpts.getRandomOption()) {
        // 生成一条事件日志
        val oneEventLog: String = initOneEventLog(oneStartupLog)
        // 发送事件日志
        Okhttp3Client.post(url, oneEventLog)
        Thread.sleep(100)
      }
      Thread.sleep(1000)
    })
